...
export default function TestimonialsPage() {
  ...
  const stats = [
    ...
    { icon: TrendingUp, label: 'Token Holders', value: '8,934+', color: 'text-green-600' },
    { icon: Shield, label: 'Security Score', value: '99.9%', color: 'text-blue-600' },
    { icon: Zap, label: 'Satisfaction Rate', value: '98.5%', color: 'text-purple-600' },
  ];
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-20 animate-fade-in-up animation-delay-200">
            {stats.map((stat, index) => (
              ...
              >
                ...
                <p className={`text-4xl font-poppins font-bold ${stat.color} mb-2`}>{stat.value}</p>
                      ...
                      <div className="glass-card p-8 h-full flex flex-col glow-border transition-all duration-700 hover:scale-105 hover:shadow-2xl hover:shadow-gold/20">
                        <div className="flex items-center gap-4 mb-6">
                          <div className="h-16 w-16 rounded-full bg-gradient-to-br from-gold/40 to-gold/15 border-2 border-gold/60 flex items-center justify-center flex-shrink-0">
                            <span className="text-gold font-poppins font-bold text-lg">{testimonial.avatar}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <h3 className="text-lg font-poppins font-bold text-gold truncate">{testimonial.name}</h3>
                            <p className="text-sm metallic-text-secondary font-inter truncate">{testimonial.role}</p>
                          </div>
                        </div>
          ...
          <div className="glass-card-gold p-12 text-center glow-border animate-fade-in-up animation-delay-400">
            <h2 className="text-4xl font-poppins font-bold text-gold mb-6">Join Our Growing Community</h2>

...
export function useMarketPulse() {
  return useQuery<MarketPulseData>({
    ...
    queryFn: async () => {
      ...
      const { histogram: macdHistogram, macdLine, signalLine } = computeMACD(closes);
      ...
      return { status, rsi, macdHistogram, macdLine, signalLine, price, change24h, lastUpdated: new Date() };
    },

...
export function useTokenAdvancedAnalytics(tokenSymbol: string) {
  return useQuery<TokenAnalytics>({
    ...
    queryFn: async () => {
      ...
      const volumes: number[] = (chartData.total_volumes ?? []).map(
        (v: number[]) => v[1],
      );
      ...
      const volumeTrend: "Rising" | "Falling" | "Stable" =
        recentVol > prevVol * 1.05
          ? "Rising"
          : recentVol < prevVol * 0.95
            ? "Falling"
            : "Stable";
      ...
      const price7dAgo =
        closes.length >= 7 ? closes[closes.length - 7] : closes[0];
      const price30dAgo = closes[0] ?? price;
      const change7d = price7dAgo
        ? ((price - price7dAgo) / price7dAgo) * 100
        : 0;
      const change30d = price30dAgo
        ? ((price - price30dAgo) / price30dAgo) * 100
        : 0;

      const ma20 =
        closes.slice(-20).reduce((a, b) => a + b, 0) /

...
import {
  useGetMarketPulseTally,
  useVoteMarketPulse,
} from "../hooks/useMarketPulseVoting";
...
export default function MarketPulseVoting() {
  ...
  const [userVote, setUserVote] = useState<Sentiment | null>(() => {
    try {
      return sessionStorage.getItem(SESSION_KEY) as Sentiment | null;
    } catch {
      return null;
    }
  });
  ...
  useEffect(() => {
    if (userVote) {
      try {
        sessionStorage.setItem(SESSION_KEY, userVote);
      } catch {
        /* ignore */
      }
    }
  ...
  const handleVote = async (sentiment: Sentiment) => {
    ...
    try {
      ...
      setVoteError(
        e instanceof Error ? e.message : "Vote failed. Please try again.",
      );
    }
  };
  ...
  const buttons: {
  ...
  }[] = [
    {
      ...
      label: "Bullish",
      icon: TrendingUp,
      activeClass: "border-emerald-500 bg-emerald-500/20 text-emerald-400",
    ...
    },
    {
      ...
      label: "Bearish",
      icon: TrendingDown,
      activeClass: "border-red-500 bg-red-500/20 text-red-400",
    ...
    },
    {
      ...
      label: "Neutral",
      icon: Minus,
      activeClass: "border-slate-400 bg-slate-500/20 text-slate-300",
      barClass: "bg-gradient-to-r from-slate-600 to-slate-400",
    },
  ];
    ...
    <div className="gman-card rounded-2xl p-6">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          {isRefetching && (
            <RefreshCw className="w-3.5 h-3.5 text-gold-accent animate-spin" />
          )}
        ...
        {buttons.map(({ sentiment, label, icon: Icon, activeClass }) => {
          ...
          return (
            ...
            >
              ...
              {isSelected && (
                <span className="text-xs font-mono opacity-75">✓ Voted</span>
              )}
            </button>
          );
        })}
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => (
            <div
              // biome-ignore lint/suspicious/noArrayIndexKey: static skeleton array has no unique identifiers
              key={i}
            ...
            return (
              <div key={sentiment}>
                <div className="flex justify-between text-xs mb-1">
                  <span
                    className={`font-semibold ${isSelected ? "text-gold-accent" : "text-slate-400"}`}
                  >
